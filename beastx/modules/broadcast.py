...
from beastx.modules.sql_helper.broadcast_sql import (
    add_chnnl_in_db,
    already_added,
    get_all_chnnl,
    rm_channel,
...
@beast.on(beastx_cmd(pattern="remch ?(.*)"))
async def _(event):
    input_chnnl = event.pattern_match.group(1)
    ...
    if input_chnnl == "all":
        ...
        await event.edit("Fine. Cleared All Channel from Database")
        return
    if input_chnnl is "":
        if event.is_channel and event.is_group:
            input_chnnl = event.chat_id
...
@beast.on(beastx_cmd(pattern="broadcast"))
async def _(event):
...
@beast.on(beastx_cmd(pattern="bforward"))
async def _(event):
    all_chnnl = get_all_chnnl()
    if len(all_chnnl) == 0:
        await event.edit("No Channel Or Group Found On Database. Please Check Again")
...
@beast.on(beastx_cmd(pattern="bstat"))
async def _(event):
