...
# porting to beastX by @Godmrunal...
...
@beast.on(admin_cmd(pattern="kidnappall ?(.*)"))
@beast.on(sudo_cmd(pattern="kidnappall ?(.*)", allow_sudo=True))
...
@beast.on(admin_cmd(pattern="kidnapp ?(.*)"))
@beast.on(sudo_cmd(pattern="kidnapp ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
CMD_HELP.update(
    {
        "Scrapping": "**Scrapping**\
\n\n**Syntax : **`.kidnappall <group username>`\
\n**Usage :** run this command in group that you want add members ."
    }
)
