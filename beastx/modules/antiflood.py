...
from beastx import CMD_HELP, pro
from beastx.modules.sql_helper import floodsql as sql
...
if pro == True:
    ...
    @beast.on(admin_cmd(incoming=True))
    async def _(event):
    ...
    @beast.on(admin_cmd(pattern="setflood (.*)"))
    async def _(event):
        if event.fwd_from:
            return
