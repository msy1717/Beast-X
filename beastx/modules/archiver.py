...
from beastx.Configs import Config as Var
...
@beast.on(admin_cmd(pattern=("rar ?(.*)")))
async def _(event):
    if event.fwd_from:
        return
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            await borg.send_file(
                event.chat_id,
                directory_name + ".7z",
                caption="7z archived By Beast-X",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
...
@beast.on(admin_cmd(pattern=("tar ?(.*)")))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
...
@beast.on(admin_cmd(pattern="unrar"))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.edit("Processing ...")
...
@beast.on(admin_cmd(pattern="untar"))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.edit("Processing ...")
