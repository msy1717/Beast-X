...
from beastx import LOGS, CMD_HELP, TEMP_DOWNLOAD_DIRECTORY
...
async def download(target_file):
    """ For .download command, download files to the beastx's server. """
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the beastx's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("beastx.session", "config.env", "beastx.modules", "*/modules/*" ):
